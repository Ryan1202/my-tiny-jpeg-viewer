use std::{cmp::min, fs::File, io::{self, BufReader, Read, Seek}};
...
impl BitReader for BufReader<File> {
    fn read_byte(&mut self, offset: usize) -> Result<u8, BitStreamErrorType> {
        let mut buf = [0u8];
        let pos = self.stream_position().map_err(|e| BitStreamErrorType::IOError(e))?;
        self.seek(io::SeekFrom::Current(offset as i64)).map_err(|e| BitStreamErrorType::IOError(e))?;
        self.read_exact(&mut buf).map_err(|e| BitStreamErrorType::IOError(e))?;
        if buf[0] == 0xff {
            self.read_exact(&mut buf).map_err(|e| BitStreamErrorType::IOError(e))?;
            if buf[0] == 0x00 {
                buf[0] = 0xff;
        ...
        self.seek(io::SeekFrom::Start(pos)).map_err(|e| BitStreamErrorType::IOError(e))?;
        Ok(buf[0])
    }
    fn remove_byte(&mut self) -> Result<u8, BitStreamErrorType> {
        let mut buf = [0u8];
...
impl<'a, R: BitReader> BitStream<'a, R> {
    pub fn new(reader: &'a mut R) -> Self {
        Self { reader: reader, cur_byte: 0, bit_start: 0 }
    }
    ...
    pub fn read(&mut self, n: usize) -> Result<usize, BitStreamErrorType> {
        ...
        while left_len > 0 {
            ...
            self.bit_start += len;
            result = (result << len) | (self.cur_byte as usize>>(8 - self.bit_start)) & ((1<<len) - 1);
            left_len -= len;
        }
        Ok(result)
    }

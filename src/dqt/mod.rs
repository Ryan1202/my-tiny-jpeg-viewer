...
impl fmt::Display for Dqt {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f, "dqt[{}]({}bit):{{", self.id, if self.precision == 0 {8} else {16})?;
        let zigzag = ZigZagScan::new(8);
        write!(f, "[")?;
        let mut i = 0;
        for (x, y) in zigzag.into_iter() {
            if self.table[[y, x]] < 10 {write!(f, " ")?;}
...
impl Dqt {
    pub fn new(map: &mut HashMap<u8, Rc<Dqt>>, length: u16, data: Vec<u8>) -> Result<(), ShapeError> {
        ...
        while len > 0 {
            ...
            map.insert(num, Rc::new(Self {id: num, precision, table: Rc::new(arr)}));

use std::{collections::HashMap, fs::File, io::{BufReader, Seek}, path::Path, rc::Rc};

use application::InterchangeFormat;
use bitstream::BitStream;
use component::{frame::{Frame, FrameErrorType}, scan::Scan, Component};
use decode::dct::DCT;
...
pub async fn get_jpeg_image(path: String) -> (usize, usize, Vec<u8>) {
    ...
    for ele in segs {
        match ele.segment_type {
            ...
            SegmentType::DHT => {
                let h = match HuffmanTable::new(ele.data) {
                    Ok(h) => {h},
                    Err(e) => {match e {
                        HuffmanErrorType::BitStreamError(_) => {panic!("In Huffman: BitStream Read Error!")},
                        HuffmanErrorType::DecodeError(bs) => {panic!("In Huffman: Decode Error! BitStream:{}", bs)},
                        HuffmanErrorType::InvalidCode(symbol, code) => {panic!("In Huffman: Invalid Code: Symbol:{}, Code:{}", symbol, code)},
                        HuffmanErrorType::TypeConversionError => {panic!("In Huffman: Type Conversion Error!")},
                    }},
                };
                match h.get_type() {
                    dht::HuffmanTableType::DC => {dc_map.insert(h.id(), Rc::new(h));},
                    dht::HuffmanTableType::AC => {ac_map.insert(h.id(), Rc::new(h));},
                }
            },
            SegmentType::SOFn(n) => {
                frame = match Frame::new(n, ele.data) {
                    Ok(f) => {Some(f)},
                    Err(e) => {match e {
                        FrameErrorType::InvalidFrameType(n) => {panic!("In Frame: Invalid Frame Type:{}", n)},
                    }},
                };
            },
    ...
    (width, height, decode::decode_image(&frame, comps, &mut bs, restart_interval, dct).unwrap())
}